...
>
  ...
  {:else}
    ...
    <div class="profile-avatar unauthenticated">
      <UserPlus size={18} class="signup-icon" />
    </div>
  {/if}
</button>
...
<style>
  .profile-button {
    ...
    /* MODIFIED: Changed border color for constant visibility */
  ...
  .profile-button:hover {
    /* MODIFIED: Added background color on hover for a clean fill */
  ...
  .profile-avatar.unauthenticated {
    /* MODIFIED: Set to transparent to allow parent hover effect */
    background-color: transparent;
    transition: background-color 0.2s ease;
  }
  
  /* REMOVED: The hover effect is now on the parent .profile-button */
  
  .profile-button:hover :global(.signup-icon) {
    /* Icon becomes white on hover for contrast */
    color: white;
  }
  
  :global(.signup-icon) {
    /* MODIFIED: Icon is red by default to match the border */
